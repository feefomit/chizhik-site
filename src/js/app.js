...
(() => {
  ...
  function ensureCityModal() {
      ...
      <div style="width:min(720px,100%);background:#fff;border-radius:16px;box-shadow:0 20px 60px rgba(0,0,0,.25);overflow:hidden;">
        ...
        <div style="padding:14px 16px;">
          ...
          <div style="border-top:1px solid #e5e7eb;padding-top:16px;">
            <input id="citySearch" placeholder="Или введите название города..." 
                   style="width:100%;padding:12px 12px;border:1px solid #e5e7eb;border-radius:12px;outline:none;">
            <div id="cityResults" style="margin-top:12px;display:grid;gap:8px;max-height:300px;overflow-y:auto;"></div>
          </div>
        </div>
      </div>
    `;
    ...
    if (quick) {
      ...
      quick.innerHTML = popular.map((n) =>
        `<button type="button" data-city="${escapeHtml(n)}"
          style="border:1px solid #e5e7eb;background:#fff;border-radius:999px;padding:6px 12px;cursor:pointer;font-weight:700;font-size:13px;transition:all 0.2s;">
          ${escapeHtml(n)}
        </button>`
      ).join("");
    ...
    $("citySearch")?.addEventListener("input", () => {
      ...
      t = setTimeout(() => {
        ...
        if (q.length < 2) {
          $("cityResults").innerHTML = `<div style="color:#6b7280;padding:12px;">Введите хотя бы 2 буквы…</div>`;
          return;
        }
  ...
  function openCityModal(prefill = "") {
    ...
    if (inp) {
      inp.value = prefill;
      if (prefill) inp.dispatchEvent(new Event("input"));
    }
  }
  ...
  async function searchCity(query) {
    ...
    if (!box) return;
    box.innerHTML = `<div style="color:#6b7280;padding:12px;">Ищу…</div>`;

    const data = await api(`/geo/cities?search=${encodeURIComponent(query)}&page=1`);
    const items = data?.items || [];

    if (!items.length) {
      box.innerHTML = `<div style="color:#6b7280;padding:12px;">Ничего не найдено.</div>`;
      return;
    }
    ...
    items.slice(0, 20).forEach((c) => {
      ...
      b.style.cssText = `
        ...
        cursor:pointer;display:flex;justify-content:space-between;gap:12px;align-items:center;transition:background 0.2s;
      `;
      b.innerHTML = `
        <div>
  ...
  function renderCategories(cats) {
    ...
    (cats || []).slice(0, 24).forEach((cat) => {
      ...
      tile.onmouseenter = () => {
        ...
        tile.style.boxShadow = "0 15px 40px rgba(17,24,39,.12)";
      };
      tile.onmouseleave = () => {
  ...
  async function loadTree() {
    ...
    if (cached) {
      ...
      setText("catHint", `Категории: ${state.city.name} (${state.mainCats.length})`);
      return;
    }
  ...
  async function loadPromo() {
    ...
    const data = await api(
      `/catalog/products?city_id=${encodeURIComponent(state.city.id)}&category_id=${state.promoCatId}&page=1`,
    ...
    setText("prodHint", discounted.length
      ? `Скидки дня (${Math.min(24, discounted.length)} товаров)`
      : `Промо товары (${Math.min(24, list.length)})`
    );
  }
  ...
  async function init() {
    ...
    try {
      ...
      console.error(e);
      setText("catHint", `Ошибка: ${e?.message || e}`);
      setText("prodHint", `Ошибка загрузки`);
    }
  }
